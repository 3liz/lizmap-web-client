...
test.describe('Form edition all field type', function () {
    ...
    test('value map with integers, dropdown menu @readonly', async function ({ page }) {
        const project = new ProjectPage(page, 'form_edition_all_field_type');
        ...
        // Set integer positive value
        let select = await page.locator('#jforms_view_edition_value_map_integer');
        let values = [];
        for (const option of await select.locator('option').all()) {
            values.push(await option.getAttribute('value'));
        }
        expect(values).toEqual(['', '1', '2', '3', '4']);
        select.selectOption('one');
        await expect(select).toHaveValue('1');
        select.selectOption('');
        await expect(select).toHaveValue('');
        select.selectOption('three');
    ...
    test('multiline text field @readonly', async function ({ page }) {
        // The html_text field must be a textarea (multiline is checked)
        await expect(page.locator('textarea#jforms_view_edition_multiline_text')).toHaveCount(1);
    });

    test('multiline HTML field, must show WYSIWIG tools @readonly', async function ({ page }) {
        // The html_text field must be a textarea (multiline is checked)
        await expect(page.locator('textarea#jforms_view_edition_html_text')).toHaveCount(1);
