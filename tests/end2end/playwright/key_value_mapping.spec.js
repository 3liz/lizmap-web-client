...
test.describe('Key/value in attribute table', () => {
    ...
    test('must display values instead of key in parent attribute table', async ({ page }) => {
        ...
        const theaders = tableWrapper.locator('div.dt-scroll-head th');
        ...
        for (const row of attribute_table_rows) {
            const tdata = tableWrapper.locator(`div.dt-scroll-body tr[id="${row.id}"] td`);
    ...
    test('must display values instead of key in children attribute table', async ({ page }) => {
        ...
        for (const row of child_table) {
            ...
            tableWrapper.locator(`div.dt-scroll-body tr[id="${row.id}"]`).click();
            ...
            // Child table headers
            const child_theaders = childTableWrapper.locator('div.dt-scroll-head th');
            ...
            // Child table data
            await expect(childTableWrapper.locator(`div.dt-scroll-body tr[id="${row.id}"]`)).toHaveCount(1);
            const child_tdata = childTableWrapper.locator(`div.dt-scroll-body tr[id="${row.id}"] td`);
    ...
    test('must display values instead of key for edition in children attribute table', async ({ page }) => {
        ...
        tableWrapper.locator('div.dt-scroll-body tr[id="1"] td lizmap-feature-toolbar .feature-edit').click();
        ...
        // Child table headers
        const child_theaders = editionChildTableWrapper.locator('div.dt-scroll-head th');
        await expect(child_theaders).toHaveCount(11);
        const child_headers = await child_theaders.allTextContents();
        expect(child_headers).toEqual(expect.arrayContaining(attribute_table_headers));
        // Child table data
        const row = attribute_table_rows[0];
        await expect(editionChildTableWrapper.locator(`div.dt-scroll-body tr[id="${row.id}"]`)).toHaveCount(1);
        const child_tdata = editionChildTableWrapper.locator(`div.dt-scroll-body tr[id="${row.id}"] td`);
        await expect(child_tdata).toHaveCount(11);
        const data = await child_tdata.allTextContents();
        expect(data).toEqual(expect.arrayContaining(row.values));
    });
