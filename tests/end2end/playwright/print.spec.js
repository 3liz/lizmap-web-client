        ...
        await page.route('**/service*', async route => {
            ...
            if (request.postData()?.includes('GetPrint')) {
                await route.fulfill({
                    ...
                    path:path.join(__dirname, 'mock/print/selection/print_labels.pdf')
                })
            } else {
                await route.continue()
            }
...
test.describe('Print in popup',
    ...
    test('Atlas print in popup requests', async ({ page }) => {
        ...
        const getPrintParams = await checkParameters('Atlas print in popup requests', getPrintRequest.postData() ?? '', expectedParameters)
