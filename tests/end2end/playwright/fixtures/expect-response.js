...
export const expect = baseExpect.extend({
    ...
    /**
     * Expecting the response is a valid image JPEG
     * @param {APIResponse|Response|null} response the response to test
     *
     * @returns {MatcherReturnType} the result
     */
    toBeImageJpeg(response) {
        const assertionName = 'toBeImageJpeg';
        let pass = response !== null;
        try {
            if (pass) {
                // check response status
    ...
    /**
     * Expecting the response is a valid image WEBP
     * @param {APIResponse|Response|null} response the response to test
     *
     * @returns {MatcherReturnType} the result
     */
    toBeImageWebp(response) {
        const assertionName = 'toBeImageWebp';
        let pass = response !== null;
        try {
            if (pass) {
                // check response status
        ...
        const message = pass
            ? () => this.utils.matcherHint(assertionName, undefined, undefined, { isNot: this.isNot }) +
                '\n\n' +
                'Response is image WEBP\n'+
                `Received: ${received}`
            : () => this.utils.matcherHint(assertionName, undefined, undefined, { isNot: this.isNot }) +
                '\n\n' +
                'Response is not image WEBP\n'+
                `Received: ${received}`;
