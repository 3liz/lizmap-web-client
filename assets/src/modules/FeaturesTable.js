...
export default class FeaturesTable {
     ...
     * @param {number|null} withGeometry
     ...
     */
    getFeatures(layerId, filter = null, withGeometry = 0, fields = 'null') {
        ...
        formData.append('with_geometry', withGeometry);
    ...
    openPopup(layerId, feature, uniqueField, targetElement, aCallBack) {
        ...
        let layerConfig = lizMap.getLayerConfigById(layerId)?.[1];
        ...
        var crs = 'EPSG:4326';
        if(('crs' in lizMap.config.layers[layerName]) && lizMap.config.layers[layerName].crs != ''){
            crs = lizMap.config.layers[layerName].crs;
        }
