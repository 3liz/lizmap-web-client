...
export default class SelectionTool {
     ...
     */
    constructor(map, digitizing, lizmap3) {
        ...
        const featureTypes = this._lizmap3.getVectorLayerFeatureTypes();
        ...
        for (const attributeLayerName in config.attributeLayers) {
            if (config.attributeLayers.hasOwnProperty(attributeLayerName)) {
                for (const featureType of featureTypes) {
                    const lname = this._lizmap3.getNameByTypeName(featureType.getElementsByTagName('Name')[0].textContent);
    ...
    // List of WFS format
    get exportFormats() {
        return this._lizmap3.getVectorLayerResultFormat().filter(
