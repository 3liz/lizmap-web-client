...
if  [ -d $BACKUPDIR ]; then
    ...
    if [ -f $BACKUPDIR/lizmapLogConfig.ini.php ]; then
        cp -p $BACKUPDIR/lizmapLogConfig.ini.php    $LIZMAP/var/config/
    fi
    if [ -f $BACKUPDIR/authldap.coord.ini.php ]; then
        cp -p $BACKUPDIR/authldap.coord.ini.php     $LIZMAP/var/config/
    fi 
    if [ -d $BACKUPDIR/lizmap-theme-config ]; then
        cp -Rp $BACKUPDIR/lizmap-theme-config/*     $LIZMAP/var/lizmap-theme-config/
    fi
