...
<form action="{formurl 'jacl2db_admin~users:index'}" method="get" class="form-inline">
...
{formurlparam 'jacl2db_admin~users:index'}
    ...
    <br/><input type="submit" value="{@jacl2db_admin~acl2.show.button@}"  class="btn"/>
</fieldset>
</form>
...
{foreach $users as $user}
    <tr class="{if $line}odd{else}even{/if}">
        ...
        <td><a href="{jurl 'jacl2db_admin~users:rights', array('user'=>$user->login)}" class="btn btn-small">{@jacl2db_admin~acl2.rights.link@}</a></td>
    </tr>
{assign $line = !$line}
{/foreach}
</tbody>
...
<div class="record-pages-list">{@jacl2db_admin~acl2.pages.links.label@} {pagelinks_bootstrap 'jacl2db_admin~users:index', array('grpid'=>$grpid, 'filter'=>$filter),  $usersCount, $offset, $listPageSize, 'idx' }</div>
{/if}
