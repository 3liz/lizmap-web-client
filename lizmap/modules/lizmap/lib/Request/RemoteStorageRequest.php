...
class RemoteStorageRequest {
         ...
         *
         * @return array{0: string, 1: int, 2: string} Array url of the uploaded file (0 :string) HTTP code(1: int), message (2: string)
         */
                  ...
                  if($profile && is_array($profile) && array_key_exists("enabled",$profile) && $profile["enabled"] == 1 && array_key_exists("baseUri",$profile)){
                           ...
                           if(strpos($storageUrl, $profile["baseUri"]) === 0){
                                    ...
                                    if($errorCode)
                                    {
                                             ...
                                             //\jLog::log("Error on file upload ".$errorMessage,"error");
                                             //return array(null,$http_code,"Error on file upload");
                                    ...
                                    else  
                                    {
                                             if($http_code >= 200 &&  $http_code <=300 ){
                                                      ...
                                                      //return array($storageUrl,$http_code,"");
                                             }
                                             else {
                                                      $message = "Error on file upload";
                                                      //\jLog::log("Error on file upload","error");
                                                      //return array(null,$http_code,"Error on file upload");
                                             }
                                    }
                           }
                                    ...
                                    //\jLog::log("Invalid path ".$storageUrl,"error");
                                    //return  array(null,400,"Error on file upload");
                           }
                  }
                  else {
                           $message = "WebDAV configuration not found";
                           //return array(null,400,"WebDAV configuration not found");
                  }
                  if($message){
