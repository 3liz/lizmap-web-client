...
class featuresCtrl extends jController
{
     ...
     * @urlparam $PROJECT Name of the project
     * @urlparam $LAYERID Layer Id
     * @urlparam $EXP_FILTER QGIS expression filter
     * @urlparam $WITH_GEOMETRY If we need to get the features geometries
     *
     * @return jResponseJson geoJSON content
     */
    public function displayExpression()
    {
        ...
        $repository = $this->param('repository');
        $layerId = trim($this->param('layerId', ''));
        $withGeometry = trim($this->intParam('with_geometry', 0));
        ...
        // Filter
        // TODO : ? protect the filter to be only like "field_name" = VALUE
        $exp_filter = '"LIBQUART" LIKE \'P%\' ';
        $exp_filter = trim($this->param('exp_filter', 'FALSE'));

        // Get the evaluated features for the given layer and parameters
        $data = qgisExpressionUtils::virtualFields(
            $qgisLayer,
            $expressions,
            $exp_filter,
            $withGeometry,
