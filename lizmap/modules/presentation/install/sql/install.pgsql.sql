-- Search path must be defined by Lizmap Web Client PostgreSQL connection profile in lizmap/var/config/profiles.ini.php

-- Table presentation
CREATE TABLE IF NOT EXISTS presentation (
    id int GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    uuid text NOT NULL DEFAULT uuid_generate_v4()::text UNIQUE,
    repository text NOT NULL,
    project text NOT NULL,
    title text NOT NULL,
    description text,
    background_image text,
    background_display text DEFAULT 'cover',
...
CREATE TABLE IF NOT EXISTS presentation_page (
    ...
    illustration_display text DEFAULT 'cover',
...
COMMENT ON TABLE presentation_page IS 'Characteristics of a presentation page';
COMMENT ON COLUMN presentation_page.id IS 'Automatic unique integer ID';
...
COMMENT ON COLUMN presentation_page.background_display IS 'Default background image position and size: contain, cover or stretched';
COMMENT ON COLUMN presentation_page.background_color IS 'Background color of the page';
COMMENT ON COLUMN presentation_page.text_color IS 'Text color of all presentation pages';
